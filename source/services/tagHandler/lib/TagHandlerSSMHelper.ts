...
/**
 * @description helper class for AWS SSM operations
 ...
 */
export class TagHandlerSSMHelper {
  /**
  ...
  static async getParameter(parameterName: string): Promise<string[]> {
    try {
      ...
      if (!response.Parameter?.Value || !response.Parameter?.Version) {
        ...
        logger.debug({
          label: "TagHandlerSSMHelper/getParameter",
  ...
  static async putParameter(
    ...
    try {
      ...
      await ssm
        ...
        .promise();
      logger.info({
        label: "TagHandlerSSMHelper/putSSMParameter",
